  ...
  $(function () {
    ...
    function addField () {
      ...
      $addField = $([
        ...
        '    <p class="part-obs">Choose the type of new field you want to create.</p>',
        ...
        '          <label for="select-u-input">Select (normal)</label>',
        '        </div> ',
        '        <div class="radio-group">',
        '          <input type="radio" id="select-m-input" value="selectM" name="in-type" required />',
        '          <label for="select-m-input">Select (multiple).</label>',
        '        </div>',
        '      </div>',
        '',
    ...
    function editField () {
      ...
      if ($zone.find('.fa-form-group').length === 0 && $this.find('i').is('.fa-pencil')) {
        alert('You must create at least one input before editing one of them!');
        return;
      }
        ...
        $this
          .find('i.fa')
            .removeClass('fa-check')
              .addClass('fa-pencil')
                .attr('title', 'Click here to edit a field.')
        ;
        ...
        .find('.input-edit-trigger')
          .on('click', function () {
            ...
            $editField = $([
              ...
              '    <h3>Edit the field...</h3>',
              '    <p class="part-obs">Edit the attributes of the field bellow:</p>',
            ...
            if (inType === 'number') {
              ...
              $editField = $([
                ...
                '        </div>',
                '        <div class="form-group">',
                '          <label for="edit-input-max">Max length of the input:</label>',
                '          <input type="text" id="edit-input-max" value="' + maxLgt + '" />',
                '        </div>',
                '        <div class="form-group">',
                '          <label for="edit-input-min">Min length of the input:</label>',
    ...
    function removeField () {
      ...
      if ($zone.find('.fa-form-group').length === 0 && $this.find('i').is('.fa-minus')) {
        alert('You must create at least one input before removing one!');
        return;
      }
      ...
      if ($this.find('i').is('.fa-check')) {
        $this
          .find('i.fa')
            .removeClass('fa-check')
              .addClass('fa-minus')
                .attr('title', 'Click here to remove a field.')
        ;
    ...
    var generate = function (type) {
      ...
      if (type === 'textarea') {
        $addField
          .find('.modal')
            .append([
              '<p class="part-obs">You have chosen a textarea!</p>',
              '<p class="part-obs">Complete the fields below with the details of your new field.</p>',
              '<form class="configure-input">',
              '  <div class="modal-padding">',
              '    <div class="form-group">',
      ...
      if (type === 'date') {
        $addField
          .find('.modal')
            .append([
              '<p class="part-obs">You have chosen a date input!</p>',
              '<p class="part-obs">Complete the fields below with the details of your new field.</p>',
              '<form class="configure-input">',
              '  <div class="modal-padding">',
              '    <div class="form-group">',
      ...
      if (type === 'color') {
        $addField
          .find('.modal')
            .append([
              '<p class="part-obs">You have chosen a color input!</p>',
              '<p class="part-obs">Complete the fields below with the details of your new field.</p>',
              '<form class="configure-input">',
              '  <div class="modal-padding">',
              '    <div class="form-group">',
      ...
      if (type === 'number') {
        $addField
          .find('.modal')
            .append([
              '<p class="part-obs">You have chosen a number input!</p>',
              '<p class="part-obs">Complete the fields below with the details of your new field.</p>',
              '<form class="configure-input">',
              '  <div class="modal-padding">',
              '    <div class="form-group">',
              ...
              '    </div>',
              '    <div class="form-group">',
              '      <label for="input-max">Max length:</label>',
              '      <input type="text" id="input-max" value="20" />',
              '    </div>',
              '    <div class="form-group">',
              '      <label for="input-min">Min length:</label>',
      ...
      if (type === 'selectU') {
        $addField
          .find('.modal')
            .append([
              '<p class="part-obs">You have chosen a normal select input!</p>',
              '<p class="part-obs">Complete the fields below with the details of your new field.</p>',
              '<form class="configure-input">',
              '  <div class="modal-padding">',
              '    <div class="form-group">',
              ...
              '    </div>',
              '    <div class="form-group">',
              '      <label for="input-vals">Field values (separate them by line break):</label>',
      ...
      if (type === 'selectM') {
        $addField
          .find('.modal')
            .append([
              '<p class="part-obs">You have chosen a multiple select input!</p>',
              '<p class="part-obs">Complete the fields below with the details of your new field.</p>',
              '<form class="configure-input">',
              '  <div class="modal-padding">',
              '    <div class="form-group">',
              '      <label for="input-id">ID (Automático):</label>',
              '      <input type="text" id="input-id" value="campo' + number + '" readonly />',
              '    </div>',
              '    <div class="form-group">',
              '      <label for="input-label">Label:</label>',
    ...
    $('#create-text-btn, #step-2, #back-to-2').on('click', function () {
      ...
      if ($('body').is('.is-editing')) {
        alert('Complete your action before! (Click the check icon in the edit sidebar.)');
        return;
      }
    ...
    var generateCode = function (codeType) {
      ...
      if (codeType === 'topiccode') {
        ...
        var POSTgeneratedCode = [
          ...
          '<!-- ->',
          '/*',
          ' * Generated in <ajuda.forumeiros.com>.',
          ' * Generator (2.0) feito por Luiz~ (http://ajuda.forumeiros.com/u60563)',
          ' */',
          '<- -->',
          '<head>',
          '  <meta charset="UTF-8">',
